...
| `navigation` | 🟡 | ⬜ | Top menu + admin nav: config-driven items, target routes, access gating (`requiresAppAdmin`); has known route/label discrepancies to fix | `src/config/nav/*`, `src/components/layout/top-menu.tsx`, `src/components/layout/header.tsx` |
